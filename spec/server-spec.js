...
describe('Persistent Node Chat Server', function() {
  ...
  it('Should insert posted messages to the DB', function(done) {
    ...
    request({
      method: 'POST',
      uri: 'http://127.0.0.1:5000/classes/users',
      json: { username: 'aplai168' }
    }, function () {
      // Post a message to the node chat server:
      request({
        method: 'POST',
        uri: 'http://127.0.0.1:5000/classes/users', ///change to messages
        json: {
          username: 'Valjean',
        ...
        dbConnection.query(queryString, queryArgs, function(err, results) {
          ...
          expect(results[0].username).to.equal('aplai168');
  ...
  it('Should output all messages from the DB', function(done) {
    ...
    dbConnection.query(queryString, queryArgs, function(err) {
      ...
      request('http://127.0.0.1:3000/classes/users', function(error, response, body) {
        ...
        // var messageLog = body;
        //delete this later
        expect(messageLog[0].username).to.equal('aplai168');
